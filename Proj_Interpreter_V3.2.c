...
int main(void)
{
    ...
    ptr = fopen("C:/Users/bvattipolu/downloads/balayya.txt", "r");
    ...
    //if statement for handling "G02" and "G03" codes only
        ...
        switch (current_plane) {
            case XY_PLANE:
                ...
                if (!i_given) {
                    if (strcmp(prev_x_pos, "n/a") != 0 && strcmp(x_pos, "n/a") != 0) {
                        float prev_x_value = atof(prev_x_pos); // Convert previous X to float
                        float current_x_value = atof(x_pos); // Convert current X to float
                        // Calculate I as the difference between current X and previous X
                        float i_value = current_x_value - prev_x_value; 
                        snprintf(i_pos, BUFFERSIZE, "%g", i_value);
                        i_given = 1;

                    } else {
                        // Handle the case where previous X or current X is not a valid number
                        printf("Error: Cannot calculate I because previous or current X is invalid.\n");
                        fprintf(fp, "Error: Cannot calculate I because previous or current X is invalid.\n");
                        check = 0;
                    }
                } else {
                    // If 'I' is given, ensure that it is the difference between the current and previous X
                    if (i_index != -1) {
                        float i_value_provided = atof(coords[i_index] + 1); // +1 to skip 'I' or 'i'
                        float i_value_calculated = atof(x_pos) - atof(prev_x_pos);
                        // If the provided 'I' does not match the calculated difference, update it
                        if (fabs(i_value_provided - i_value_calculated) > 0.001) { // Using a small threshold for floating point comparison
                        // snprintf(i_pos, BUFFERSIZE, "%g", i_value_calculated);
                        //     i_given = 1;   
                        printf("Error: Cannot calculate I because current X is invalid.\n");
                        fprintf(fp, "Error: Cannot calculate I because current X is invalid.\n");
                        check = 0;          
                        }
                    }
                }
                ...
                if (!j_given) {
                    if (strcmp(prev_y_pos, "n/a") != 0 && strcmp(y_pos, "n/a") != 0) {
                        float prev_y_value = atof(prev_y_pos); // Convert previous Y to float
                        float current_y_value = atof(y_pos); // Convert current Y to float
                        // Calculate J as the difference between current Y and previous Y

                        float j_value = current_y_value - prev_y_value; 
                        snprintf(j_pos, BUFFERSIZE, "%g", j_value);
                        j_given = 1;

                    } else {
                        // Handle the case where previous Y or current Y is not a valid number
                        printf("Error: Cannot calculate J because previous or current Y is invalid.\n");
                        fprintf(fp, "Error: Cannot calculate J because previous or current Y is invalid.\n");
                        check = 0;
                    }
                } else {
                    // If 'J' is given, ensure that it is the difference between the current and previous Y
                  
                    if (j_index != -1) {
                        float j_value_provided = atof(coords[j_index] + 1); // +1 to skip 'J' or 'j'
                        float j_value_calculated = atof(y_pos) - atof(prev_y_pos);
                        // If the provided 'J' does not match the calculated difference, update it
                        if (fabs(j_value_provided - j_value_calculated) > 0.001) { // Using a small threshold for floating point comparison
                        // snprintf(i_pos, BUFFERSIZE, "%g", j_value_calculated);
                        //     j_given = 1;   
                        printf("Error: Cannot calculate J because current Y is invalid.\n");
                        fprintf(fp, "Error: Cannot calculate J because current Y is invalid.\n");
                        check = 0;          
                        }
                    }
                }

                if((i_given && j_given) || (strcmp(i_pos, "n/a") != 0 && strcmp(j_pos, "n/a") != 0) ){
                    if((i_given == j_given)||(i_pos == j_pos)){
                        float j_value = 0;
                        snprintf(j_pos, BUFFERSIZE, "%g", j_value);
                        j_given = 1;
                    }
